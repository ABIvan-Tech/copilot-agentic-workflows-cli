...
- Session context may hold current-plan notes, transient routing hints, or short-lived user preferences.
- Never treat ephemeral session state as canonical project truth.
- If a detail must survive across sessions or be shared with future agents, write it into `.agent-memory/`.

> **OVERRIDE — Copilot Memory is NOT the canonical repo memory layer here.**
> Do NOT treat Copilot Memory as the durable source of truth for this repository.
> All repo-scoped durable facts go exclusively to `.agent-memory/` files because they are git-tracked, reviewable, and portable.
...
3. route durable memory writes to `memory-curator` unless the user explicitly chose a different flow
